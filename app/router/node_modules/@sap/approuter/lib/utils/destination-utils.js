    ...
    tenantDestinations.forEach((tenantDestination) => {
      getDestinationPromises.push(
        new Promise((resolve, reject) => {
          ...
          getDestinationsByDestinationKey(req.app, destinationKey,tenantDestination.migrated, headerUtils.getCorrelationId(req), false, function (err, destinations) {
            if (err) {
              reject(err);
            } else {
...
async function getDestinationsByDestinationKey(app, destinationKey,migrated, correlationId, runtimeFlow, cb) {
  ...
  if (!destinationToken || destinationToken.tokenRefreshTimestamp - Date.now() <= 0 || !app.services[destinationKey].credentials) {
    ...
    const destMigrated = migrated || process.env.MIGRATED_DEST;
