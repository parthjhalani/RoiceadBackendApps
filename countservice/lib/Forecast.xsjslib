...
async function executeForecast(site, tankfilter) {
    ...
    for (var j = 0; j < groupedTanks.length; j++) {
        if (groupedTanks[j].isSelected) {
            ...
            for (var i = 0; i < tanks.length; i++) {
                ...
                if (!(deltaInventories.length === 0 && firstTimeForecast)) {
                    if (deltaInventories.length > 0) {
                        var saveInventories = await saveInventoryInForecastTable(deltaInventories, tankdetails);
            ...
            await createReplenishmentOrder(noOfOrders);
        }
    }
    //connectn.commit();
	...
	/* Order Creation */
	async function getReplenishmentOrderNumber() {
	...
	async function createReplenishmentOrder(noOfOrders) {
	...
	async function deleteForecastedInventoriesOlderThanCurrTime(site, tank) {
		//var delQ = "delete from MY_ROICEAD_FORECASTCALCULATION where site='" + site + "' and tank ='" + tank + "' and Entry_Type='FCT' and ForecastedTimestamp  >= ( select current_timestamp from dummy) " ;
		var delQ = "delete from MY_ROICEAD_FORECASTCALCULATION where site='" + site + "' and tank ='" + tank +
	...
	async function saveOldRORD(tank){
		...
		for(var k=0; k < tank.Orders.length ; k++ ){
				var st = await conn.prepareStatement('INSERT INTO MY_ROICEAD_FORECASTCALCULATION VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
				...
				st.setDate(7, formatDate(suppl_date)); //ForecastDate
				st.setTime(8, formatTime(suppl_date), 'HH:MI:SS'); //ForecastTinme
				...
				st.setTimestamp(21, formatDateTime(currInvDate)); //ForecastedDateTime
				//st.addBatch();
	...
	async function executeForecastSimultaneously(groupedTanks){
		...
		while(!stopForecast){
			...
			if (hitSFBInPrevLoop === x) {
				...
				st.setDate(7, formatDate(curInvDateT)); //ForecastDate
				st.setTime(8, formatTime(curInvDateT), 'HH:MI:SS'); //ForecastTinme
				st.setString(9, 'ORD'); //ENTRY_TYPE
				st.setString(10, groupedTanks.Tanks[i].Inventory.MATERIALID); //MATERIAL
				st.setString(11, ' '); // Tankgrp
				...
				st.setDecimal(19, tankdetails.MQUAN); //Forecast Inv
				st.setString(20, forecast_event); //FORECAST_EVT
				st.setTimestamp(21, formatDateTime(curInvDateT)); //ForecastedDateTime
				st.addBatch();
			...
			for(var k=0; k < groupedTanks.Tanks[i].Orders.length ; k++ ){
				...
				st.setString(5, groupedTanks.Tanks[i].Inventory.SITE);
				st.setString(6, groupedTanks.Tanks[i].Inventory.TANKNUM);
				st.setDate(7, formatDate(currInvDate)); //ForecastDate
				st.setTime(8, formatTime(currInvDate), 'HH:MI:SS'); //ForecastTinme
				st.setString(9, 'RORD'); //ENTRY_TYPE
				st.setString(10, groupedTanks.Tanks[i].Inventory.MATERIALID); //MATERIAL
				st.setString(11, ' '); // Tankgrp
				st.setInteger(12, critical_tank); //0==false, 1 ==true CRITICAL_TANK
			...
			if ((currDate.getTime() <  groupedTanks.Tanks[i].Inventory.currInvDate.getTime()) || x === 0) {
				...
				st.setString(5, groupedTanks.Tanks[i].Inventory.SITE);
				st.setString(6, groupedTanks.Tanks[i].Inventory.TANKNUM);
				st.setDate(7, formatDate(groupedTanks.Tanks[i].Inventory.currInvDate)); //ForecastDate
				st.setTime(8, formatTime(groupedTanks.Tanks[i].Inventory.currInvDate), 'HH:MI:SS'); //ForecastTinme
				...
				st.setDecimal(19, tankdetails.MQUAN); //Forecast Inv
				st.setString(20, forecast_event); //FORECAST_EVT
				st.setTimestamp(21, formatDateTime(groupedTanks.Tanks[i].Inventory.currInvDate)); //ForecastedDateTime
				st.addBatch();
	...
	async function saveInventoryInForecastTable(inventories, tankdetails) {
		var conn = await $.db.getConnection();
		var currTimestampQuery = 'SELECT CURRENT_TIMESTAMP  FROM DUMMY';
		var currTstmpResult = await connectn.executeQuery(currTimestampQuery);
		var currTstmp = currTstmpResult[0].CURRENT_TIMESTAMP;
	...
	async function getDeltaInventories(site, tank, lastForecastDateTime, firstTimeForecast) {
		...
		currTstmp = new Date(currTstmp.setDate(currTstmp.getDate() - 10));
		currTstmp = formatDate(currTstmp);
