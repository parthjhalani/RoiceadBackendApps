...
var Reconciliation = {
	getReconcNumber : async function () {
		console.log('in getReconcNumber');
	...
	getTanksWithReconciliation : async function (sites,SitesFilter,startDate,endDate,siteType,period){
		...
		var currDateR = await connectn.executeQuery(currDateQ);
		if(!endDate || endDate === ""){
			endDate = currDateR[0].CURRENT_DATE;
		}
		//var tankQuery = "Select * from  where SITE = '" + Sites + "' and status = 'ACTIVE'" ;
		
		var result = await connectn.executeQuery(sitesQuery);
		...
		for(var i=0 ; i<result.length; i++){
			...
			var reconciliationResult = await connectn.executeQuery(reconciliationQuery);
			...
			if(!result[i].RECONCDATE || result[i].RECONCDATE === null){
				...
				var reconcD = Reconciliation.formatDateTime(result[i].RECONCDATE);
			...
			var orderRes = await connectn.executeQuery(orderquery);
			result[i].MQUAN = 0;
			...
			var metersAlloted = await connectn.executeQuery(meterAllotedToTanksQuery);
	...
	"(SELECT MAX(MeasureDate) AS maxUpdatedAt FROM MY_ROICEAD_METERREADINGS  WHERE  Site = '" + Site +"'  AND METERNUM = '" + metersAlloted[k].METERNUM + 	"'" + measureDateFilter + " GROUP BY  EXTRACT ( DAY FROM MEASUREDATE), EXTRACT ( MONTH FROM MEASUREDATE) ) as Lookup " +
		...
		*/
		...
		if(period !== "All" && period !== ""){
			diff = Reconciliation.getDiffQtyNPercent(period,Site,result[i].TANKNUM);	
			result[i].CALCDIFFQTY = diff.QTY;
			result[i].CALCDIFFPERCENT = diff.PERCENT;
			result[i].PERIOD = diff.period;
		}else if(period === "All"){
			var diff1 = Reconciliation.getDiffQtyNPercent("currmonth",Site,result[i].TANKNUM);
			result[i].CURRDIFFQTY = diff1.QTY;
			result[i].CURRDIFFPERCENT = diff1.PERCENT;
			
			var diff2 = Reconciliation.getDiffQtyNPercent("prevmonth",Site,result[i].TANKNUM);
			result[i].PREVDIFFQTY = diff2.QTY;
			result[i].PREVDIFFPERCENT = diff2.PERCENT;
			
			var diff3 = Reconciliation.getDiffQtyNPercent("quarter",Site,result[i].TANKNUM);
			result[i].QRTRDIFFQTY = diff3.QTY;
			result[i].QRTRDIFFPERCENT = diff3.PERCENT;
			
			var diff4 = Reconciliation.getDiffQtyNPercent("year",Site,result[i].TANKNUM);
	...
	getDiffQtyNPercent : async function (period, site, tanknum){
		var qstring = "";
		var shortp = "";
		var connectn = await $.hdb.getConnection();
		if(period !== "All"){
		...
		var queryResp = await connectn.executeQuery(query);
		if(queryResp[0]){
